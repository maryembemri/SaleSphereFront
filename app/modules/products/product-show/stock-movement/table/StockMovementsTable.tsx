...
import {StockMovementsPagination} from '../components/pagination/StockMovementsPagination'
import {StockMovementsLoading} from '../components/loading/StockMovementsLoading'
        ...
        >
          <thead>
            <tr className='text-start text-muted fw-bolder fs-7 text-uppercase '>
              {headers.map((column: ColumnInstance<StockMovement>) => (
          ...
          <tbody className='text-gray-800 fw-semibold' {...getTableBodyProps()}>
            {rows.length > 0 ? (
              rows.map((row: Row<StockMovement>, i) => {
                prepareRow(row)
